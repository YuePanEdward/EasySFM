...
#include "extra_tool.h"
#include "estimate_motion_eg.h"

using namespace p3dv;

bool EpipolarEstimator::estimateE8Points(std::vector<cv::KeyPoint> &keypoints1,
                                         ...
                                         Eigen::Matrix4f &T)
{
    ...
    essential_matrix = findEssentialMat(pointset1, pointset2, focal_length, principal_point);
    std::cout << "essential_matrix is " << std::endl << essential_matrix << std::endl;
    
    cv::Mat R;
    cv::Mat t;

    recoverPose(essential_matrix, pointset1, pointset2, R, t, focal_length, principal_point);
...
bool EpipolarEstimator::estimateE5Points(std::vector<cv::KeyPoint> &keypoints1,
                                         std::vector<cv::KeyPoint> &keypoints2,
                                         std::vector<cv::DMatch> &matches,
                                         Eigen::Matrix3f &K,
                                         Eigen::Matrix4f &T)
