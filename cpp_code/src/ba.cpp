...
namespace p3dv
{
    ...
    bool BundleAdjustment::solveBA(double fix_calib_tolerance_BA)
    {
        ...
        if (fix_calib_tolerance_BA == 0) // only camera pose and 3d points are set as parameters
        {
            for (int i = 0; i < num_observations_; i++)
            {
                ...
                if (camera_index_[i] == ref_process_camera_id_)
                {
                    for (int j = 0; j < 6; j++)
                    {
                        ceres_problem_->SetParameterLowerBound(mutable_camera_for_observation(i), j, -fixed_threshold);
                        ceres_problem_->SetParameterUpperBound(mutable_camera_for_observation(i), j, fixed_threshold);
                    }
                }
            }
        }
        else // camera intrinsic parameters are also set as parameters
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                ceres_problem_->SetParameterLowerBound(mutable_calib(), i, parameters_[num_parameters_ - (4 - i)] - fix_calib_tolerance_BA);
                ceres_problem_->SetParameterUpperBound(mutable_calib(), i, parameters_[num_parameters_ - (4 - i)] + fix_calib_tolerance_BA);
            }
        }
